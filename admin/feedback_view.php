    ...
    <div id="responsive_tbl" style="width: 70%; margin-left: 15%;"> 
      ...
      <div style="overflow-x:auto;">
        <table>
          ...
          <?php 
                $selectfeedback="Select * from Feedback f,Customer c where f.customerid=c.customerid";
                $runselectfeedback=mysqli_query($connection,$selectfeedback);
                $countoffeedback=mysqli_num_rows($runselectfeedback);
                if ($countoffeedback>0) {
                  for ($i=0; $i <$countoffeedback ; $i++) { 
                    $data=mysqli_fetch_array($runselectfeedback);
                    $feedbackid=$data['feedbackid'];
                    $feedbackcustomerid=$data['customerid'];
                    $feedbackcustomername=$data['customername'];
                    $feedback=$data['feedback'];
                    echo "<tr>
                      <td>$feedbackid</td>
                      <td>$feedbackcustomerid</td>
                      <td>$feedbackcustomername</td>
                      <td>$feedback</td>
                      <td>
                      <a href='feedback_delete.php?fid=$feedbackid'>Delete</a> 
                      </td>
                    </tr>
                    ";
                  }
